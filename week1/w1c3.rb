...
def multiplication_table(integer, heading = '', decorate = false)
  ...
  max_integer = integer * integer # Max integer on the table
  item_length = max_integer.to_s.length + 1 # Length of all table items
  line_offset = item_length > 2 ? 0 : 1 # Left line offset for short items
  line_length = line_offset + (integer == 0 ? 2 : integer * item_length) #Length lines (0 special case)
  heading_offset = line_length / 2 - heading.length / 2 #Calculates heading offset to center it
  heading_offset = 1 if heading_offset < 1
  ...
  if integer != 0
    ...
    line << ' ' * line_offset << "% #{item_length}d" % integer << "\n"
  end
  line << ' ' << '=' * line_length << "\n" if decorate
  line # returns line
end
...
# Output test
(0..10).each{|x| puts multiplication_table x, "Table #{x}", true}
